...
def clean_template():
    ...
    output_path = r'C:\Users\ortho\.openclaw\workspace\PortalX LITE\Settings\PortalX_LITE_template_clean_optimized.json'
    ...
    for sheet_name, sheet in data["metadata"]["sheets"].items():
        ...
        visible_columns = {
            col: dim for col, dim in sheet.get("column_dimensions", {}).items()
            if not dim.get("hidden", False)
        }
        
        # Keep only row 1 (header row)
        ...
        cleaned_sheet = {
            ...
            "column_dimensions": visible_columns,
            "row_dimensions": header_row_dims,
